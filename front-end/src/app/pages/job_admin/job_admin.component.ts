...
@Component({
    ...
    templateUrl: './job_admin.component.html',
    styleUrls: ['./job_admin.component.css']
  })
  ...
  export class JobAdminComponent implements OnInit {
    ...
    ngUpdateDeadline(deadline: HTMLInputElement){
        let job = {
            id : this.job.id,
            isim : "",
            status : -1,
            ...
            point: -1,
            parent: -1,
            deadline: deadline.value as unknown as Date
        }
        if(deadline.value == null || deadline.value == "")
            alert("Deadline boş olamaz");
        else{
            this.JobService.updateDeadline(job).subscribe((response) => { 
                if(response.message.endsWith(" olarak güncellendi.")){
                  location.assign("/job/admin");
                }
                else
    ...
    ngInactive(){
      if(this.job.status == 4)
        alert("İşin statüsü zaten inaktif.")
      else{
        if(this.job.status != 3){
          this.users?.forEach(user => {
            this.UserService.updatePoint(user, this.job.point).subscribe((response) =>{
        ...
        this.job.status = 4;
        this.JobService.statusChange(this.job).subscribe((response) => { 
          if(response.message == this.job.isim+" isimli işin statüsü İnaktif olarak güncellendi.")
            location.assign("/job");
        });
      }
    }

    ngUpdate(point: HTMLInputElement){
      this.job.point = point.value as unknown as number;
      if(this.job.status == 3){
        alert("Tamamlanmış işin puanı güncellenemez.");
      }
      else{
        this.JobService.updatePoint(this.job).subscribe((response) => { 
          if(response.message == "Puan güncellendi.")
            location.assign("/job");
        });
      }
    }
    ...
    ngWorking(){
      ...
      else if(this.job.status == 3 || this.job.status == 4){
        this.users?.forEach(user => {
          let modifier = -1;
   ...
   async ngPrint(){
      ...
      let button7 = document.getElementById("addSubJob");
